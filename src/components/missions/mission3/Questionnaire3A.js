...
const COMPONENT_ID = 'questionnaire3a';
...
const getPAGES = (questionVersion) => [
    ...
    questions: [
      {
        ...
        type: 'accordion-likert',
        required: true,
        questions: questionVersion === 'A' ? EP_NORMAL : EP_VARIED
      }
    ]
  },
...
const Questionnaire3A = () => {
  ...
  const {questionVersion } = useUserStats();
  const PAGES = getPAGES(questionVersion);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (group === '0') navigate('/mission3/stroop-test');
      else if (group === '1') navigate('/mission3/intervention-a');
      else if (group === '2') navigate('/mission3/intervention-b');
...
export default Questionnaire3A;
